...
async def add_user_to_server(username: str, hashed_password: str, group: str, server: str, database: str, user: str, password: str) -> None:
    ...
    if not user_exists:
        ...
        cur.execute(f"CREATE USER {username} WITH ENCRYPTED PASSWORD '{hashed_password}'")

    else:
        # Update password with hashed password
        hashed_password = hashlib.sha256(hashed_password.encode()).hexdigest()
        cur.execute(f"ALTER USER {username} WITH ENCRYPTED PASSWORD '{hashed_password}'")
...
async def remove_user_from_server(username: str, group: str, host: str, database: str, user: str, password: str) -> None:
    conn = psycopg2.connect(
        host=host,
        database=database,
        user=user,
    ...
    cur.execute(f"""SELECT oid FROM pg_roles WHERE oid IN (
    ...
    if not is_member_of_other_groups:
        ...
        cur.execute(f"SELECT relname, relkind FROM pg_catalog.pg_class WHERE relowner = (SELECT oid FROM pg_roles WHERE rolname='{username}')")
        objects_owned = cur.fetchall()

        for obj in objects_owned:
            # Reassign ownership to database owner
            cur.execute(f"SELECT datdba FROM pg_catalog.pg_database WHERE datname = '{database}'")
            result = cur.fetchone()
...
async def get_local_group_members(server_name: str, group: str) -> List[str]:
    ...
    if response.status_code == 200:
        ...
        print(f"Error getting local group members for group {group} on server {server_name}")
        return []
...
async def process_server(server: str) -> None:
    ...
    for group in local_groups:
        ...
        print(f"Group {group}: {len(members_to_add)} members to add, {len(members_to_remove)} members to remove")
