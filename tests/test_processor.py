...
from app.services.processor import add_user_to_server, remove_user_from_server, sync_roles, get_servers, get_local_groups, get_local_group_members, get_external_group_members, process_servers, process_server
...
def test_add_user_to_server():
    ...
    asyncio.run(add_user_to_server(username, hashed_password, group, server, database, user, password))
...
def test_remove_user_from_server():
    ...
    asyncio.run(remove_user_from_server(username, group, server, database, user, password))
...
def test_process_server(mock_external_members, mock_local_members):
    ...
    mock_print.assert_called_with("Group testgroup: 1 members to add, 1 members to remove")